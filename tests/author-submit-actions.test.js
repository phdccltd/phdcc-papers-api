...
describe('AUTHOR', () => {
  it('submit actions', async () => {
    ...
    try {
      ...
      error = await runscript.run(app.models, 'tests/api-add-paper-docx-author.json', false, app)
      if (error) throw new Error(error)

      error = await runscript.run(app.models, 'tests/api-author-submit-actions.json', false, app)
      if (error) throw new Error(error)
