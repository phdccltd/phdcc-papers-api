...
describe('OWNER', () => {
  it('gradings actions', async () => {
    ...
    try {
      ...
      error = await runscript.run(app.models, 'tests/api-owner-not-gradings-actions.json', false, app)
      if (error) throw new Error(error)

      error = await runscript.run(app.models, 'tests/api-login-owner1.json', false, app)
      if (error) throw new Error(error)

      error = await runscript.run(app.models, 'tests/api-owner-gradings-actions.json', false, app)
      if (error) throw new Error(error)
