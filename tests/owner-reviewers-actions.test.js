...
describe('OWNER', () => {
  it('reviewers actions', async () => {
    ...
    try {
      ...
      error = await runscript.run(app.models, 'tests/api-owner-not-reviewers-actions.json', false, app)
      if (error) throw new Error(error)

      error = await runscript.run(app.models, 'tests/api-login-owner1.json', false, app)
      if (error) throw new Error(error)

      error = await runscript.run(app.models, 'tests/api-owner-reviewers-actions.json', false, app)
      if (error) throw new Error(error)
