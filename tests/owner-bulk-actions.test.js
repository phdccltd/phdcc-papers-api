...
describe('OWNER', () => {
  it('bulk actions', async () => {
    ...
    try {
      ...
      error = await runscript.run(app.models, 'tests/api-bulk-bad1.json', false, app)
      if (error) throw new Error(error)
      ...
      error = await runscript.run(app.models, 'tests/api-bulk-bad2.json', false, app)
      if (error) throw new Error(error)
